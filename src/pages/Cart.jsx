...
const Cart = () => {
  ...
  return (
    <div className='flex flex-col gap-2 md:px-8 px-5 md:py-8 py-5'>
      <h1 className='font-semibold text-xl'>Order Details</h1>
      {/* display added items here  */}
      <div className='w-full border my-2'></div>

      <div className='flex flex-col gap-2 no-scrollbar overflow-y-scroll  h-[calc(90vh-150px)] pb-10'>
        {
          items.length > 0 ? (
            items.map((item, index) => (
              <div className="flex flex-col bg-white shadow-sm border gap-2 py-2 px-2 rounded-md" key={index}>
                {/* card */}
                <div className="flex gap-2">
                  <img src={item.image} alt="imageurl" width={100} className="rounded-sm" />
                  <div className="flex flex-col justify-between w-full">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <button className="p-1 bg-blue text-white w-fit rounded-full" onClick={() => handleClick(item)}>
                        <MdDelete />
                      </button>
                    </div>
                  </div>
      ...
      <div className='relative bottom-0 fixed py-2 flex-col w-full'>
        ...
        <button className='bg-blue text-white font-medium text-md py-2 w-full rounded-md' onClick={proceedToPay}>Proceed To Pay</button>
      </div>
